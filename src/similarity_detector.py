...
def check_new_joke(new_joke, jokes, embeddings, model, threshold=0.45):
    """Check if a new joke is similar to existing jokes."""
    new_embedding = model.encode([new_joke])[0]
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        new_joke = " ".join(sys.argv[1:])

        print(f"Checking joke: \"{new_joke}\"\n")
        ...
        results = check_new_joke(new_joke, jokes, embeddings, model)

        print("Top 5 Similar Jokes:\n")
        for i, r in enumerate(results, 1):
            flag = "SUSPICIOUS" if r['suspicious'] else ""
            print(f"{i}. Score: {r['score']:.3f} {flag}")
            print(f"    {r['joke']['comedian']} ({r['joke']['year']})")
            print(f"    \"{r['joke']['joke'][:80]}...\"\n")
    else:
        # Load the model
        ...
        high_pairs = [x for x in cross_similarities if x['score'] >= 0.45]
